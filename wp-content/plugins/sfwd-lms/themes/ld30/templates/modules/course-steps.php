...
if( !empty($learndash_previous_nav) || !empty($learndash_next_nav) || !empty($complete_button) ): ?>
	...
	<div class="ld-content-actions">
		...
		<div class="ld-content-action<?php if ( !$learndash_previous_nav ): ?> ld-empty<?php endif; ?>">
			<?php
			if( $learndash_previous_nav ): ?>
					<a class="<?php echo esc_attr($button_class); ?>" href="<?php echo esc_attr($learndash_previous_nav); ?>">
						<span class="ld-icon ld-icon-arrow-left"></span>
						<span class="ld-text"><?php echo sprintf( esc_html_x( 'Previous %s', 'Previous Item Text', 'learndash' ), learndash_get_content_label(get_post_type()) ); ?></span>
					</a>
			<?php endif; ?>
		</div>
		...
		if( $parent_id && $context != 'focus' ): ?>
			...
			<a href="<?php echo esc_attr(learndash_get_step_permalink( $parent_id, $course_id )); ?>" class="ld-primary-color"><?php echo sprintf( esc_html_x( 'Back to %s', 'Back to Item Text', 'learndash' ), learndash_get_content_label( get_post_type(), array( 'parent' => true ) ) ); ?></a>
		...
		<div class="ld-content-action<?php if (( ! $can_complete ) && ( ! $learndash_next_nav )) : ?> ld-empty<?php endif; ?>">
			...
			elseif($learndash_next_nav): ?>
					<a class="<?php echo esc_attr($button_class); ?>" href="<?php echo esc_attr($learndash_next_nav); ?>">
						<span class="ld-text"><?php echo sprintf( esc_html_x( 'Next %s', 'Previous Item Text', 'learndash' ), learndash_get_content_label(get_post_type()) ); ?></span>
