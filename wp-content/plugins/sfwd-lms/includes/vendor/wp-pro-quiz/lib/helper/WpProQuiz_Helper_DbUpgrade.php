...
class WpProQuiz_Helper_DbUpgrade {
	...
	public function upgrade($version) {
		...
		if ( ( defined('LEARNDASH_ACTIVATED')) || ( $version === false ) || ( (int)$version > WpProQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION ) ) {
			$this->install();
...
*/
	...
	private function upgradeDbV21() {
		...
		$this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS  `'.$this->_wpdb->prefix.'wp_pro_quiz_form` (
			  `form_id` int(11) NOT NULL AUTO_INCREMENT,
			  `quiz_id` int(11) NOT NULL,
			  `fieldname` varchar(100) NOT NULL,
			  `type` tinyint(4) NOT NULL,
		...
		$this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS  `'.$this->_wpdb->prefix.'wp_pro_quiz_template` (
			  `template_id` int(11) NOT NULL AUTO_INCREMENT,
			  `name` varchar(200) NOT NULL,
			  `type` tinyint(3) unsigned NOT NULL,
			  `data` text NOT NULL,
