...
if ( !class_exists( 'LearnDash_BitBucket_API' ) ) {
	class LearnDash_BitBucket_API {
		...
		function get_download_base_url() {
			return $this->download_url_base;
		}

		function get_readme_base_url() {
		...
		function get_bitbucket_repositories() {
		...
		function get_bitbucket_repositories_ORG() {
			...
			if ( !empty( $request_url ) ) {
				...
				if ( ( $code === 200 ) && ( !empty( $body ) ) ) {
					...
					if ( ( property_exists( $repositories, 'values' ) ) && ( is_array( $repositories->values ) ) && ( !empty( $repositories->values ) ) ) {
						
						foreach( $repositories->values as $plugin ) {
							if ( property_exists( $plugin, 'slug' ) ) {
								//if ( in_array( $plugin->slug, array( 'plugin-test-1', 'plugin-test-2' ) ) ) {
									$repository_data[$plugin->slug] = $plugin;	
								//}
							}
						}
					}
		...
		function get_bitbucket_repository_readme( $plugin_key = '' ) {
			if ( !empty( $plugin_key ) ) {
		...
		// Retreive the readme.txt file from Amazon S3.
		// Filenamess should be in the form format <repo slug>_readme.txt
		// as in learndash-gravity-forms_readme.txt or learndash-edd_readme.txt
		function get_bitbucket_repository_readme_S3( $plugin_key = '' ) {
			if ( !empty( $plugin_key ) ) {
